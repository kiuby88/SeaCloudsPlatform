 ...
 * under the License.
 */
package brooklyn.entity.openshift;
...
public abstract class OpenShiftEntityImpl  extends AbstractEntity implements OpenShiftEntity,
    ...
    private transient OSPaasEntityDriver driver;
    ...
    @Override
    public OSPaasEntityDriver getDriver() {
        return driver;
    }
    ...
    protected void initDriver(OpenShiftPaasLocation location) {
        OSPaasEntityDriver newDriver = doInitDriver(location);
        if (newDriver == null) {
     ...
     */
    protected OSPaasEntityDriver doInitDriver(OpenShiftPaasLocation location) {
        if (driver != null) {
    ...
    protected OSPaasEntityDriver newDriver(OpenShiftPaasLocation loc) {
        EntityDriverManager entityDriverManager = getManagementContext().getEntityDriverManager();
        return (OSPaasEntityDriver) entityDriverManager.build(this, loc);
    }
